...
import json
import datetime
from dateutil.parpiser import parse
...
def str_to_time(timestamp,tzsource=None):
    ...
    try:
        parsed_datetime = parse(timestamp)
    ...
    if tzsource:
        if isinstance(tzsource, str):
            timezone = pytz.timezone(tzsource)
            localized_datetime = timezone.localize(parsed_datetime)
            return localized_datetime
        elif isinstance(tzsource, datetime.datetime):
            return parsed_datetime.replace(tzinfo=tzsource.tzinfo)
...
def daytime(time,daycycle):
    ...
    if year not in daycycle or month_day not in daycycle[year]:
        return None

    sunrise_str = daycycle[year][month_day]["sunrise"]
    sunset_str = daycycle[year][month_day]["sunset"]

    timezone_str = daycycle["timezone"]
    timezone = pytz.timezone(timezone_str)
    
    sunrise_time = datetime.datetime.strptime(year + "-" + month_day + " " + sunrise_str, '%Y-%m-%d %H:%M')
    sunset_time = datetime.datetime.strptime(year + "-" + month_day + " " + sunset_str, '%Y-%m-%d %H:%M')
